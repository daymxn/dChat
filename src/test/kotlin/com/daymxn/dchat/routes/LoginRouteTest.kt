    ...
    "request validation" -
      {
        "fails when missing username" {
          val response = callHandler(LoginRequest("", USER_PASSWORD_TESTS))
        ...
        "fails when username does not exists" {
          val localUser = GenerateMockObjects.localUser()
          testManager.markUserAsNotFound(localUser)

          val response = callHandler(LoginRequest(localUser.username, localUser.password))
          response.error shouldNotBe null
        }
        "fails when password is incorrect" {
          val localUser = GenerateMockObjects.localUser()
          testManager.saveUser(localUser)

          val response = callHandler(LoginRequest(localUser.username, localUser.password + "xyz"))
          response.error shouldNotBe null
        }
        "fails on internal database error" {
          coEvery { UserService.getByUsername(any()) } answers
            {
              UnknownDatabaseError(mockk()).asFailure()
            }

          val response = callHandler(LoginRequest(USER_USERNAME_TESTS, USER_PASSWORD_TESTS))
          response.error shouldNotBe null
        }
      }
